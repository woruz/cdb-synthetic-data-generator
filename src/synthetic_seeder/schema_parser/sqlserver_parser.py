"""SQL Server / T-SQL DDL parser. Handles [bracketed] identifiers, IDENTITY, SQL Server types."""
...
def _parse_table_block(block: str) -> TableDef | None:
    ...
    if not name_match:
        # Unquoted table name
        name_match = re.match(r"([\w]+)\s*\(?", first, re.IGNORECASE)
    if not name_match:
        return None
    raw = name_match.group(1)
    if "." in raw:
        table_name = _norm(raw.split(".")[-1])
    else:
        table_name = _norm(raw)
    ...
    for line in lines[1:]:
        ...
        if "REFERENCES" in upper or upper.startswith("FOREIGN KEY"):
            ...
            continue
        col_match = re.match(r"^(\[[\w]+\]|\w+)\s+(\S+(?:\s*\([^)]*\))?)\s*(.*)$", line, re.IGNORECASE)
        if col_match:
...
def _parse_fk(line: str) -> ForeignKeyDef | None:
    ...
    if fk_match:
        ...
        col_match = re.match(r"^(\[[\w]+\]|\w+)\s+", line, re.IGNORECASE)
...
def parse_sqlserver_schema(schema_content: str) -> NormalizedSchema:
    """Parse SQL Server / T-SQL DDL into NormalizedSchema."""
